...
public class JDBCDataSource {
	...
	public static void main(String[] args) {
		...
		JavaPairRDD<String, Tuple2<Integer, Integer>> studentsRDD =
				studentInfosDF.javaRDD().mapToPair(f ->{
					Tuple2 t =new Tuple2<String, Integer>(f.getString(0),
							Integer.valueOf(String.valueOf(f.get(1))));
					return t;
				})
				.join(studentScoresDF.javaRDD().mapToPair(m ->{
					Tuple2 t =new Tuple2<String, Integer>(String.valueOf(m.get(0)),
							Integer.valueOf(String.valueOf(m.get(1))));
					return t;
				}));
		
		// 将JavaPairRDD转换为JavaRDD<Row>
		JavaRDD<Row> studentRowsRDD = studentsRDD.map(line ->{
			 return  RowFactory.create(line._1, line._2._1, line._2._2);
		});
