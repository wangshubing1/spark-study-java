...
public class NewsOfflineStatSpark {
	...
	public static void main(String[] args) {
		...
		// 开发第四个关键指标：用户跳出率统计
		calculateDailyUserJumpRate(spark, yesterday);
		// 开发第五个关键指标：版块热度排行榜
		calculateDailySectionPvSort(spark, yesterday);
	...
	private static void calculateDailyPagePv(
			SparkSession spark, String date) {
		String sql = 
				"SELECT "
					+ "date,"
					+ "pageid,"
	...
	private static void calculateDailyNewUserRegisterRate(
		...
		// 执行两条SQL，获取结果
		Object result1 = spark.sql(sql1).collect()[0].get(0);
		long number1 = 0L;
		if(result1 != null) {
			number1 = Long.valueOf(String.valueOf(result1));  
		}
		
		Object result2 = spark.sql(sql2).collect()[0].get(0);
		long number2 = 0L;
		if(result2 != null) {
