...
public class RDD2DataFrameProgrammatically {
	...
	public static void main(String[] args) {
		...
		JavaRDD<String> lines = spark.sparkContext()
				.textFile("C://Users//Administrator//Desktop//students.txt",1)
				.toJavaRDD();
		...
		JavaRDD<Row> studentRDD = lines.map((Function<String, Row>) record ->{
			String[] lineSplited = record.split(",");
		...
		// 第三步，使用动态构造的元数据，将RDD转换为DataFrame
		 Dataset studentDF = spark.createDataFrame(studentRDD, structType);
