...
public class TransformBlacklist {
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        JavaPairDStream<String, String> userAdsClickLogDStream =
                adsClickLogDStream.mapToPair(s -> new Tuple2<>(s.split(" ")[1], s));
        ...
        JavaDStream<String> validAdsClickLogDStream = userAdsClickLogDStream.transform(
                userAdsClickLogRDD -> {
                    ...
                    JavaPairRDD<String, Tuple2<String, Optional<Boolean>>> filteredRDD =
                            joinedRDD.filter(f -> {
                                if (f._2._2().isPresent() && f._2._2.get()) {
                                    return false;
                                }
                                return true;
                            });
                    ...
                    // 进行map操作，转换成我们想要的格式
                    JavaRDD<String> validAdsClickLogRDD = filteredRDD.map(m -> m._2._1);
